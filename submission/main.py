...
# Subprocess runner code template (all training logic inline, no imports from main)
RUNNER_CODE = r'''
...
def train_split(split_dir, experiment):
...
if __name__ == '__main__':
    ...
    train_split(split_dir, experiment)
'''
...
def process_split_subprocess(split_dir, experiment):
    """Process one split in a subprocess to fully isolate memory.

    Writes a temp runner script and executes it. If the subprocess crashes
    (e.g. segfault), falls back to inline train_and_predict with CB-only.
    """
    # Write the runner script to a temp file
    runner_path = os.path.join(tempfile.gettempdir(), f'split_runner_{os.getpid()}.py')
...
def main():
    ...
    for i, split_dir in enumerate(data_dirs):
        ...
        if row_ids is None:
            # Fallback: try inline with CB-only
            print(f"  Retrying with CB-only inline...")
            try:
                train = pd.read_csv(os.path.join(split_dir, 'train.csv'))
                test = pd.read_csv(os.path.join(split_dir, 'test.csv'))
                row_ids, preds = train_and_predict(train, test, experiment='cb')
                del train, test
            except Exception as e:
