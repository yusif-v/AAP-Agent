...
def get_adaptive_params(n):
    if n < 500:
        return dict(n_seeds=1, cb_iterations=500, cb_depth=4, cb_lr=0.05, xgb_n_est=200, xgb_depth=3, xgb_lr=0.03, et_n_est=100, et_depth=4)
    elif n < 2000:
        return dict(n_seeds=3, cb_iterations=1000, cb_depth=5, cb_lr=0.03, xgb_n_est=300, xgb_depth=4, xgb_lr=0.03, et_n_est=200, et_depth=6)
    elif n < 10000:
        return dict(n_seeds=3, cb_iterations=1500, cb_depth=6, cb_lr=0.03, xgb_n_est=500, xgb_depth=4, xgb_lr=0.03, et_n_est=300, et_depth=8)
    else:
        return dict(n_seeds=1, cb_iterations=500, cb_depth=6, cb_lr=0.1, xgb_n_est=200, xgb_depth=5, xgb_lr=0.1, et_n_est=200, et_depth=6)
...
def get_model(name):
    if name == 'et':
        return ExtraTreesClassifier(n_estimators=adaptive['et_n_est'], max_depth=adaptive['et_depth'], random_state=42, n_jobs=-1)
    if name == 'xgb':
        return EarlyStoppingXGB(n_estimators=adaptive['xgb_n_est'], max_depth=adaptive['xgb_depth'], learning_rate=adaptive['xgb_lr'], random_state=42, n_jobs=-1, verbosity=0)
    if name == 'lr':
...
for name in models_to_train:
    try:
        ...
        model = get_model(name)
        model.fit(X_df, y)
        pred = model.predict_proba(cb_X_test if name == 'cb' else X_test)[:, 1]
        predictions.append(pred)
        model_ids.append(name)
        del model
        gc.collect()
...
'''
    # Write runner to temp file and execute
    ...
    if result.returncode != 0:
        print(f"  Subprocess failed (exit {result.returncode}): {result.stderr[-500:]}")
        return None, None
