...
across all 16 data splits and writes a combined submission.csv.

Each split is processed in a subprocess to isolate memory and prevent
segfaults from accumulation across 16 splits. If a subprocess segfaults
(exit -11), retries with a sklearn-only runner (no CatBoost/XGBoost imports).
"""
...
# Runner code templates for subprocess execution.
# Written to temp files to avoid f-string escaping issues.

# Full ensemble runner: ET+XGB+LR+CB (imports all libraries)
...
n = len(y)
if n < 500:
    n_seeds = 1; cb_it = 300; cb_depth = 4; cb_lr = 0.1; xgb_ne = 100; xgb_d = 3; xgb_lr = 0.1; et_ne = 100; et_d = 4
elif n < 2000:
    n_seeds = 1; cb_it = 500; cb_depth = 5; cb_lr = 0.05; xgb_ne = 200; xgb_d = 4; xgb_lr = 0.05; et_ne = 200; et_d = 6
elif n < 10000:
    n_seeds = 1; cb_it = 500; cb_depth = 6; cb_lr = 0.03; xgb_ne = 300; xgb_d = 4; xgb_lr = 0.03; et_ne = 300; et_d = 8
else:
    n_seeds = 1; cb_it = 500; cb_depth = 6; cb_lr = 0.1; xgb_ne = 200; xgb_d = 5; xgb_lr = 0.1; et_ne = 200; et_d = 6
...
def get_model(name):
    if name == 'et':
        return ExtraTreesClassifier(n_estimators=int(et_ne), max_depth=int(et_d), random_state=42, n_jobs=2)
    if name == 'xgb':
        return EarlyStoppingXGB(n_estimators=int(xgb_ne), max_depth=int(xgb_d), learning_rate=xgb_lr, random_state=42, n_jobs=2, verbosity=0)
    if name == 'lr':
        return make_pipeline(StandardScaler(), LogisticRegression(max_iter=5000, C=0.1, random_state=42))
    if name == 'cb':
        cb_seeds = (42, 142, 242, 342, 442)[:n_seeds]
        return SeedAveragedCatBoost(cat_features=cat_feature_indices, seeds=cb_seeds, iterations=int(cb_it),
                                   depth=int(cb_depth), learning_rate=cb_lr, thread_count=2, allow_writing_files=False, save_snapshot=False)
...
for name in models_to_train:
    try:
        ...
        print("  " + name + ": CV=" + str(round(np.mean(scores), 4)))
...
result = {'row_ids': test_row_ids.tolist(), 'preds': final_pred.tolist()}
print('RESULT_JSON_START' + json.dumps(result) + 'RESULT_JSON_END')
sys.stdout.flush()
'''

# Sklearn-only runner: ET+LR only (NO CatBoost or XGBoost imports at all)
SKLEARN_ONLY_CODE = r'''import sys, os, json, warnings, gc
warnings.filterwarnings("ignore")
import pandas as pd
import numpy as np
import itertools, re
...
if len(predictions) >= 2:
    ...
    if len(sorted_idx) >= 2 and (oof_aucs[sorted_idx[0]] - oof_aucs[sorted_idx[1]]) < 0.005:
        ...
        weights = weights / weights.sum()
    oof_blend_auc = roc_auc_score(y, oof_matrix @ weights)
    print("  OOF blend: " + str(dict(zip(names, weights.round(3)))) + ", OOF AUC=" + str(round(oof_blend_auc, 4)))
    final_pred = np.column_stack(predictions) @ weights
else:
    final_pred = predictions[0]
    print("  Single model: " + model_ids[0])
...
def _write_and_run(runner_code, split_dir, suffix):
    """Write runner_code to a temp file and execute it as a subprocess."""
    runner_path = os.path.join(tempfile.gettempdir(), f'split_runner_{os.getpid()}_{id(split_dir)}_{suffix}.py')
    with open(runner_path, 'w') as f:
        f.write(runner_code)
    try:
        result = subprocess.run(
            [sys.executable, runner_path, split_dir],
    ...
    if result.returncode != 0:
        if result.returncode == -11:
            print(f"  Subprocess segfaulted (exit -11)")
        else:
            stderr_msg = result.stderr[-300:] if result.stderr else "(no stderr)"
    ...
    if start == -1 or end == -1:
        print(f"  No result marker in subprocess output")
...
def process_split(split_dir):
    """Process one split with fallback chain for segfaults.

    1. Full ensemble (ET+XGB+LR+CB) — best quality
    2. Sklearn-only (ET+LR) — if full ensemble segfaults (exit -11)

    Returns (row_ids, preds) or (None, None) if all tiers fail.
    """
    # Tier 1: Full ensemble
    result = _write_and_run(RUNNER_CODE, split_dir, 'full')
    if result is not None:
        return result

    # Tier 2: Sklearn-only fallback
    print(f"  Retrying with sklearn-only (ET+LR)...", flush=True)
    result = _write_and_run(SKLEARN_ONLY_CODE, split_dir, 'sklearn')
    if result is not None:
        print(f"  sklearn-only fallback OK", flush=True)
        return result

    print(f"  All fallback tiers failed for {split_dir}", flush=True)
    return None
...
def main():
    ...
    for i, split_dir in enumerate(data_dirs):
        print(f"\n[{i+1}/{len(data_dirs)}] Processing {split_dir}...", flush=True)

        result = process_split(split_dir)
        if result is None:
            print(f"  FAILED for {split_dir}", flush=True)
            continue

        row_ids, preds = result
        all_row_ids.extend(row_ids)
        all_preds.extend(preds)
        print(f"  OK ({len(preds)} rows)", flush=True)
        del result, row_ids, preds
        gc.collect()
